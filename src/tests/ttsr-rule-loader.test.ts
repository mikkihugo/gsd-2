...
function makeTmpProject(): { cwd: string; globalDir: string; projectDir: string; cleanup: () => void } {
	...
	const globalDir = join(cwd, '.sf-global', 'agent', 'rules')
...
// a cwd that has .sf/rules/.

// ═══════════════════════════════════════════════════════════════════════════
// Project-local rule loading
// ═══════════════════════════════════════════════════════════════════════════

test('loads rule from project .sf/rules/', (t) => {
...
test('returns empty array when .sf/rules/ does not exist', (t) => {
	const { cwd, cleanup } = makeTmpProject()
 t.after(() => { cleanup() });

		// cwd exists but no .sf/rules/ dir
		const rules = loadRules(cwd)
