...
import { SettingsManager, getAllowedCommandPrefixes, SAFE_COMMAND_PREFIXES, setAllowedCommandPrefixes } from "@sf-run/pi-coding-agent";
...
describe("applySecurityOverrides — env var and settings precedence", () => {
  ...
  beforeEach(() => {
    ...
    savedEnv.SF_ALLOWED_COMMAND_PREFIXES = process.env.SF_ALLOWED_COMMAND_PREFIXES;
    savedEnv.SF_FETCH_ALLOWED_URLS = process.env.SF_FETCH_ALLOWED_URLS;
    delete process.env.SF_ALLOWED_COMMAND_PREFIXES;
    delete process.env.SF_FETCH_ALLOWED_URLS;
  ...
  it("env var overrides settings for command prefixes", () => {
    process.env.SF_ALLOWED_COMMAND_PREFIXES = "age,infisical";
    const sm = SettingsManager.inMemory({ allowedCommandPrefixes: ["sops", "doppler"] });
    applySecurityOverrides(sm);
  ...
  it("empty env var does not override settings (falls through to settings)", () => {
    process.env.SF_ALLOWED_COMMAND_PREFIXES = "";
  ...
  it("env var with whitespace and trailing commas is trimmed correctly", () => {
    process.env.SF_ALLOWED_COMMAND_PREFIXES = " sops , doppler , , ";
  ...
  it("env var overrides settings for fetch allowed URLs", () => {
    process.env.SF_FETCH_ALLOWED_URLS = "my-docs.internal";
  ...
  it("empty env var does not override settings for fetch URLs", () => {
    process.env.SF_FETCH_ALLOWED_URLS = "";
  ...
  it("env var with whitespace and trailing commas is trimmed correctly for URLs", () => {
    process.env.SF_FETCH_ALLOWED_URLS = " a.internal , b.internal , , ";
    const sm = SettingsManager.inMemory();
    applySecurityOverrides(sm);
