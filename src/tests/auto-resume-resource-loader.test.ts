...
test("loader.ts sets SF_PKG_ROOT env var", () => {
  ...
  assert.ok(
    loaderSrc.includes("process.env.SF_PKG_ROOT"),
    "loader.ts must set SF_PKG_ROOT so deployed extensions can locate package-root modules",
  );
});
...
test("SF_PKG_ROOT resolves resource-loader.js correctly from package root", () => {
  // Simulate what auto.ts does: given SF_PKG_ROOT, construct the path
