...
function withFakeRtk<T>(mapping: Record<string, string | { status?: number; stdout?: string }>, run: () => Promise<T> | T): Promise<T> | T {
  ...
  const finalize = () => {
    ...
    if (previousDisabled === undefined) delete process.env.SF_RTK_DISABLED;
...
function withManagedFakeRtk<T>(mapping: Record<string, string | { status?: number; stdout?: string }>, run: (env: NodeJS.ProcessEnv, managedPath: string) => Promise<T> | T): Promise<T> | T {
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    SF_HOME: managedHome,
    SF_RTK_REWRITE_TIMEOUT_MS: "20000",
  };
...
// tests below verify the SF-layer surfaces that still call rewriteCommandWithRtk
...
test("shared RTK helper falls back to the managed RTK path when SF_RTK_PATH is unset", async () => {
