...
describe("extractExtensionKey", () => {
  ...
  it("extracts directory name from a nested extension path", () => {
    assert.equal(
      extractExtensionKey("/home/user/.sf/agent/extensions/mcp-client/index.js", extensionsDir),
      "mcp-client",
    );
  });
  ...
  it("strips .ts/.js suffix from flat extension files", () => {
    assert.equal(
      extractExtensionKey("/home/user/.sf/agent/extensions/my-ext.ts", extensionsDir),
      "my-ext",
    );
  });
...
describe("detectExtensionConflicts", () => {
  ...
  it("reproduces issue #2075: bundled extension under /.sf/agent/extensions/ was never identified as built-in", () => {
    ...
    const conflicts = detectExtensionConflicts(extensions, bundledKeys, "/home/user/.sf/agent/extensions");
