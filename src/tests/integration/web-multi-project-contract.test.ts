...
function makeWorkspaceFixture(label: string): { projectCwd: string; sessionsDir: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), `sf-multi-project-${label}-`));
            ...
            tasks: [
              {
                ...
                planPath: ".sf/milestones/M001/slices/S01/tasks/T01-PLAN.md",
              },
            ],
          },
        ],
...
test("multi-project: getProjectBridgeServiceForCwd returns distinct instances for different project paths", async (t) => {
  ...
  bridge.configureBridgeServiceForTests({
    env: {
      ...process.env,
      SF_WEB_PROJECT_CWD: fixtureA.projectCwd,
      SF_WEB_PROJECT_SESSIONS_DIR: fixtureA.sessionsDir,
...
test("multi-project: resolveProjectCwd falls back to SF_WEB_PROJECT_CWD when no ?project= present", (t) => {
  bridge.configureBridgeServiceForTests({
    env: {
      ...process.env,
      SF_WEB_PROJECT_CWD: "/fallback/path",
      SF_WEB_PACKAGE_ROOT: repoRoot,
    },
    spawn: createHarness("unused").spawn,
    indexWorkspace: async () => fakeWorkspaceIndex(),
