...
test('package hooks declare a concrete staged web host', () => {
  ...
  assert.equal(rootPackage.scripts['sf:web'], 'npm run build:pi && npm run copy-resources && node scripts/build-web-if-stale.cjs && node scripts/dev-cli.js --web')
  assert.equal(rootPackage.scripts['sf:web:stop'], 'node scripts/dev-cli.js web stop')
...
test('cli.ts branches to web mode before interactive startup and preserves cwd-scoped launch inputs', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-cli-'))
  ...
  assert.deepEqual(launchInputs, {
    ...
    agentDir: join(process.env.HOME || '', '.sf', 'agent'),
    host: undefined,
    port: undefined,
  ...
  assert.deepEqual(spawnInvocation, {
    ...
    options: {
      ...
      env: {
        ...
        HOSTNAME: '127.0.0.1',
        PORT: '45123',
        SF_WEB_HOST: '127.0.0.1',
        SF_WEB_PORT: '45123',
        SF_WEB_AUTH_TOKEN: authToken,
        SF_WEB_PROJECT_CWD: '/tmp/current-project',
        SF_WEB_PROJECT_SESSIONS_DIR: '/tmp/.sf/sessions/--tmp-current-project--',
...
test('runWebCliBranch handles "web stop" subcommand without --web flag', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-branch-stop-'))
  const pidFilePath = join(tmp, 'web-server.pid')
  let stderrOutput = ''
...
test('sf web <path> is handled as web start with path', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-path-'))
...
test('sf web start <path> resolves path and launches', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-start-path-'))
...
test('sf --web <path> resolves path and launches', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-flag-path-'))
...
test('sf --web <nonexistent-path> fails with clear error', async () => {
  let stderrOutput = ''

  const flags = cliWeb.parseCliArgs(['node', 'dist/loader.js', '--web', '/tmp/nonexistent-sf-test-path-xyz'])
  const result = await cliWeb.runWebCliBranch(flags, {
    stderr: { write: (chunk: string) => { stderrOutput += chunk; return true } },
  })
...
test('launch failure surfaces status and reason before browser open', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-missing-host-'))
  let openedUrl = ''
  let stderrOutput = ''
...
test('registerInstance and readInstanceRegistry round-trip', (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-registry-'))
...
test('unregisterInstance removes a single entry', (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-unreg-'))
  const registryPath = join(tmp, 'web-instances.json')
...
test('sf web stop all is parsed and dispatched', async () => {
...
test('sf web stop <path> is parsed and dispatched with resolved path', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-stop-path-'))
  let stopOptions: { projectCwd?: string; all?: boolean } | undefined
...
test('resolveContextAwareCwd returns project cwd when inside a project under dev root', (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-ctx-aware-'))
...
test('launchWebMode kills stale instance for same cwd before spawning', async (t) => {
  const tmp = mkdtempSync(join(tmpdir(), 'sf-web-stale-'))
  const standaloneRoot = join(tmp, 'dist', 'web', 'standalone')
  const serverPath = join(standaloneRoot, 'server.js')
  mkdirSync(standaloneRoot, { recursive: true })
  writeFileSync(serverPath, 'console.log("stub")\n')
  ...
  const status = await webMode.launchWebMode(
    {
      cwd,
      projectSessionsDir: '/tmp/.sf/sessions/stale',
      agentDir: '/tmp/.sf/agent',
      packageRoot: tmp,
    },
    {
  ...
  const status = await webMode.launchWebMode(
    {
      ...
      projectSessionsDir: '/tmp/.sf/sessions/clean',
      agentDir: '/tmp/.sf/agent',
      packageRoot: tmp,
    },
    {
