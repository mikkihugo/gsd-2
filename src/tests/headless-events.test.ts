 ...
 * transitive @sf-run/native import that breaks in test environment.
 */
...
test('--events parses comma-separated event types into a Set', () => {
  const opts = parseHeadlessArgs(['node', 'sf', 'headless', '--events', 'agent_end,extension_ui_request', 'auto'])
...
test('--events implies --json', () => {
  const opts = parseHeadlessArgs(['node', 'sf', 'headless', '--events', 'agent_end', 'auto'])
...
test('--events with all common types', () => {
  ...
  const opts = parseHeadlessArgs(['node', 'sf', 'headless', '--events', types, 'auto'])
  assert.equal(opts.eventFilter!.size, 5)
})

test('--events combined with other flags', () => {
  const opts = parseHeadlessArgs(['node', 'sf', 'headless', '--timeout', '60000', '--events', 'agent_end', '--verbose', 'next'])
