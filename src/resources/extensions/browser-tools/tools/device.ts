		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			try {
				...
				if (currentUrl && currentUrl !== "about:blank") {
					await page.goto(currentUrl, { waitUntil: "domcontentloaded", timeout: 15000 }).catch((e) => { if (process.env.SF_DEBUG) console.error("[browser-tools] device goto restore failed:", e.message); });
				}
