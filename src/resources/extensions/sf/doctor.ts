...
import { resolveMilestoneFile, resolveMilestonePath, resolveSliceFile, resolveSlicePath, resolveTaskFile, resolveTasksDir, milestonesDir, sfRoot, relMilestoneFile, relSliceFile, relTaskFile, relSlicePath, relSfRootFile, resolveSfRootFile, relMilestonePath } from "./paths.js";
import { deriveState, isMilestoneComplete } from "./state.js";
import { invalidateAllCaches } from "./cache.js";
import { loadEffectiveSFPreferences, type SFPreferences } from "./preferences.js";
...
function validatePreferenceShape(preferences: SFPreferences): string[] {
  const issues: string[] = [];
...
export async function runSFDoctor(basePath: string, options?: { fix?: boolean; dryRun?: boolean; scope?: string; fixLevel?: "task" | "all"; isolationMode?: "none" | "worktree" | "branch"; includeBuild?: boolean; includeTests?: boolean }): Promise<DoctorReport> {
  ...
  if (!existsSync(milestonesPath)) {
    const report: DoctorReport = { ok: issues.every(i => i.severity !== "error"), basePath, issues, fixesApplied, timing: { git: gitMs, runtime: runtimeMs, environment: envMs, sfState: 0 } };
  ...
  const report: DoctorReport = {
    ...
    issues,
    fixesApplied,
    timing: { git: gitMs, runtime: runtimeMs, environment: envMs, sfState: Math.max(0, Date.now() - t0env - envMs) },
  };
  await appendDoctorHistory(basePath, report);
  return report;
}
