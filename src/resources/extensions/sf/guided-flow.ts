...
import {
  sfRoot, milestonesDir, resolveMilestoneFile, resolveMilestonePath,
  resolveSliceFile, resolveSlicePath, resolveSfRootFile, relSfRootFile,
  ...
  if (!state.activeMilestone?.id) {
    ...
    if (pendingAutoStartMap.has(basePath)) {
      ...
      const milestoneHasContext = existsSync(
        join(sfRoot(basePath), "milestones", entry.milestoneId, `${entry.milestoneId}-CONTEXT.md`),
      );
