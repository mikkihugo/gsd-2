...
function _reconcileWorktreeLogsInner(
  ...
  if (conflicts.length > 0) {
    ...
    logError("reconcile", `${conflicts.length} conflict(s) detected on ${conflictSummary}${truncated}. Details: .sf/CONFLICTS.md`, { count: String(conflicts.length), path: join(mainBasePath, ".sf", "CONFLICTS.md") });
    return { autoMerged: 0, conflicts };
  }
