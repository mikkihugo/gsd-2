...
test("revertExecutorResolvedCaptures reverts captures resolved without classification", () => {
  ...
  const capPath = join(tmp, ".sf", "CAPTURES.md");
...
test("executeBacktrack writes trigger and regression markers", () => {
  ...
  mkdirSync(join(tmp, ".sf", "milestones", "M003"), { recursive: true });
  ...
  const triggerPath = join(tmp, ".sf", "BACKTRACK-TRIGGER.md");
  ...
  const regressionPath = join(tmp, ".sf", "milestones", "M003", "M003-REGRESSION.md");
