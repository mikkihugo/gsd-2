...
test('test block at line 76', () => {
  ...
  const raw = JSON.parse(readFileSync(join(base, '.sf', 'QUEUE-ORDER.json'), 'utf-8'));
...
test('test block at line 90', () => {
  const base = createFixtureBase();
  writeFileSync(join(base, '.sf', 'QUEUE-ORDER.json'), 'not json');
...
test('test block at line 98', () => {
  const base = createFixtureBase();
  writeFileSync(join(base, '.sf', 'QUEUE-ORDER.json'), '{"order": "invalid"}');
