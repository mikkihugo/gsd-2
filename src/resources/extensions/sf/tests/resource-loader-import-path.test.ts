...
// relative path that breaks when deployed to ~/.sf/agent/extensions/sf/.
...
describe("resource-loader import path", () => {
  test("must not use relative import reaching above extensions/", () => {
    ...
    // This resolves to ~/.sf/resource-loader.js from deployed location, which
    ...
    assert.ok(
      ...
      "breaks when deployed to ~/.sf/agent/extensions/sf/ (see #3899)",
    );
  });
