...
test("registerQueryTools registers sf_milestone_status tool", () => {
  const pi = makeMockPi();
  registerQueryTools(pi);
  assert.equal(pi.tools.length, 1, "Should register exactly one tool");
  assert.equal(pi.tools[0].name, "sf_milestone_status");
});

test("sf_milestone_status has promptGuidelines mentioning prohibited alternatives", () => {
  const pi = makeMockPi();
  registerQueryTools(pi);
...
test("sf_milestone_status returns milestone metadata and slice statuses", async () => {
...
test("sf_milestone_status returns empty slices array for milestone with no slices", async () => {
  const base = makeTmpBase();
  try {
    openTestDb(base);
...
test("sf_milestone_status returns not-found for missing milestone", async () => {
  const base = makeTmpBase();
  try {
    openTestDb(base);
...
test("sf_milestone_status handles missing DB gracefully", async () => {
  // Create a directory without .sf/ to ensure ensureDbOpen has nothing to open
