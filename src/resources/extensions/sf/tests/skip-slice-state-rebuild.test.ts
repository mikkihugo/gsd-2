/**
 * Regression test for #3477: sf_skip_slice tool must rebuild STATE.md
...
test("sf_skip_slice tool calls rebuildState after DB update (#3477)", () => {
  ...
  assert.ok(
    ...
    "sf_skip_slice must call rebuildState after updating slice status",
  );
});
...
test("rethink prompt warns against markdown-only edits for skip (#3477)", () => {
  ...
  assert.ok(
    prompt.includes("MUST") && prompt.includes("sf_skip_slice"),
    "Rethink prompt must emphasize sf_skip_slice tool requirement",
  );
});
