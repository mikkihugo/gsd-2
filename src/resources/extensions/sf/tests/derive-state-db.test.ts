...
`;
  ...
  test('derive-state-db: replanning-slice via DB', async () => {
    ...
    try {
      ...
      const { _getAdapter } = await import('../sf-db.ts');
...
`;
  ...
  test('derive-state-db: queued milestone row survives sf_plan_milestone INSERT OR IGNORE', async () => {
    try {
      openDatabase(':memory:');

      // Simulates sf_milestone_generate_id inserting a minimal queued row
      ...
      // Simulates sf_plan_milestone calling insertMilestone (INSERT OR IGNORE)
