...
describe('worktree-integration', async () => {
  ...
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", "alpha"), "sf/alpha/M001/S01", "explicit worktree param");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01"), "sf/M001/S01", "no worktree param = plain branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "sf/alpha/M001/S01", "worktree-namespaced slice branch");
  ...
  assert.ok(!branches.includes("sf/alpha/M001/S01"), "slice branch cleaned up");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "sf/alpha/M001/S02", "on S02 namespaced branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt2.path), "sf/beta/M001/S01", "beta has its own namespaced branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "sf/alpha/M001/S01", "alpha re-created S01");

  // Both exist simultaneously
  const allBranches = run("git branch", base);
  assert.ok(allBranches.includes("sf/alpha/M001/S01"), "alpha S01 branch exists");
  assert.ok(allBranches.includes("sf/beta/M001/S01"), "beta S01 branch exists");
