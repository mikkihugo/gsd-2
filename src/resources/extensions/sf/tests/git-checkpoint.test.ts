...
describe("git-checkpoint rollback", () => {
  ...
  it("cleans up checkpoint ref after rollback", (t) => {
    ...
    // Ref should exist
    const refBefore = git(["for-each-ref", "refs/sf/checkpoints/unit-3", "--format=%(objectname)"], repo);
    assert.equal(refBefore, sha);

    rollbackToCheckpoint(repo, "unit-3", sha);

    // Ref should be cleaned up
    const refAfter = git(["for-each-ref", "refs/sf/checkpoints/unit-3", "--format=%(objectname)"], repo);
  ...
  it("cleanupCheckpoint removes the ref without error", (t) => {
    ...
    const ref = git(["for-each-ref", "refs/sf/checkpoints/unit-4", "--format=%(objectname)"], repo);
