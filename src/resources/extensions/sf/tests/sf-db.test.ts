...
function tempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sf-db-test-'));
...
describe('sf.db', () => {
  ...
  test('sf-db: fresh DB schema init (memory)', () => {
  ...
  test('sf-db: double-init idempotency', () => {
  ...
  test('sf-db: insert + get requirement', () => {
    openDatabase(':memory:');
    insertRequirement({
      id: 'R007',
      class: 'functional',
  ...
  test('sf-db: active_decisions view excludes superseded', () => {
    openDatabase(':memory:');
  ...
  test('sf-db: active_requirements view excludes superseded', () => {
    openDatabase(':memory:');
  ...
  test('sf-db: WAL mode on file-backed DB', () => {
    const dbPath = tempDbPath();
    openDatabase(dbPath);
  ...
  test('sf-db: mmap stays disabled on darwin file-backed DBs', () => {
  ...
  test('sf-db: transaction rollback on error', () => {
    openDatabase(':memory:');
  ...
  test('sf-db: recreates missing verification evidence dedup index after removing duplicate rows', () => {
    const dbPath = tempDbPath();
    openDatabase(dbPath);
  ...
  test('sf-db: rowToTask tolerates legacy comma-separated task arrays', () => {
    openDatabase(':memory:');
  ...
  test('sf-db: query wrappers return null/empty when DB unavailable', () => {
    // Ensure DB is closed
    closeDatabase();
  ...
  test('sf-db: closeDatabase resets wasDbOpenAttempted after an intentional close', () => {
    openDatabase(':memory:');
  ...
  test('sf-db: rowToTask tolerates corrupt comma-separated task arrays', () => {
    openDatabase(':memory:');
