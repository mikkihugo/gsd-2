/**
 * Regression test for #2879: sf_plan_milestone silently drops milestone title
 ...
 * (INSERT OR IGNORE). When sf_plan_milestone is called later with a title,
...
test("upsertMilestonePlanning updates title when DB row pre-exists with empty title (#2879)", () => {
  try {
    ...
    // Step 2: Simulate sf_plan_milestone — insertMilestone is called again
