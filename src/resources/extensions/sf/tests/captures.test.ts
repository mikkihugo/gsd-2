...
test("captures: appendCapture creates CAPTURES.md on first call", (t) => {
  ...
  assert.ok(
    existsSync(join(tmp, ".sf", "CAPTURES.md")),
    "CAPTURES.md should exist",
  );
  const content = readFileSync(join(tmp, ".sf", "CAPTURES.md"), "utf-8");
...
test("captures: resolveCapturesPath returns .sf/CAPTURES.md for normal path", () => {
  const base = join(tmpdir(), "cap-test-project");
  const result = resolveCapturesPath(base);
  assert.ok(result.endsWith(join(".sf", "CAPTURES.md")));
  assert.ok(result.startsWith(base));
});

test("captures: resolveCapturesPath resolves worktree path to project root", () => {
  const base = join(tmpdir(), "cap-test-project");
  const worktreePath = join(base, ".sf", "worktrees", "M004");
  const result = resolveCapturesPath(worktreePath);
  assert.ok(
    result.endsWith(join(".sf", "CAPTURES.md")),
    `should end with .sf/CAPTURES.md, got: ${result}`,
  );
