 ...
 * from the main repo's .sf/ into the worktree's .sf/ for the
 * specified milestone, and deletes sf.db so it rebuilds from fresh state.
 *
 ...
 *   - sf.db deleted in worktree after sync
 ...
 *   - syncWorktreeStateBack syncs root-level .sf/ files (REQUIREMENTS, PROJECT, etc.)
 ...
 *   - syncSfStateToWorktree syncs non-standard milestone dir names (#1547)
...
import { syncSfStateToWorktree, syncWorktreeStateBack } from '../auto-worktree.ts';
...
describe('worktree-sync-milestones', async () => {
  ...
  console.log('\n=== 1. milestone directory synced from main to worktree ===');
  {
    ...
    try {
      ...
      assert.ok(!existsSync(join(wtBase, '.sf', 'milestones', 'M001')), 'M001 missing before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001')), '#1311: M001 synced to worktree');
      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001', 'M001-CONTEXT.md')), 'M001 CONTEXT synced');
      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001', 'M001-ROADMAP.md')), 'M001 ROADMAP synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 2. missing slices within milestone are synced ===');
  {
    ...
    try {
      ...
      const wtM001Dir = join(wtBase, '.sf', 'milestones', 'M001');
      mkdirSync(join(wtM001Dir, 'slices', 'S01'), { recursive: true });
      writeFileSync(join(wtM001Dir, 'slices', 'S01', 'S01-PLAN.md'), '# S01 Plan');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001', 'slices', 'S02')), '#1311: S02 synced');
      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001', 'slices', 'S02', 'S02-PLAN.md')), 'S02 PLAN synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 3. empty sf.db deleted in worktree after sync ===');
  {
    ...
    try {
      ...
      assert.ok(existsSync(join(wtBase, '.sf', 'sf.db')), 'sf.db exists before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(!existsSync(join(wtBase, '.sf', 'sf.db')), '#853: empty sf.db deleted after sync');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 3b. non-empty sf.db preserved in worktree after sync (#2815) ===');
  {
    ...
    try {
      ...
      writeFileSync(join(wtBase, '.sf', 'sf.db'), 'migrated-db-content');
      assert.ok(existsSync(join(wtBase, '.sf', 'sf.db')), 'sf.db exists before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.sf', 'sf.db')), '#2815: non-empty sf.db preserved after sync');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 7. milestones/ directory created in worktree when missing ===');
  {
    ...
    try {
      // Worktree has .sf/ but NO milestones/ subdirectory
      ...
      assert.ok(!existsSync(join(wtBase, '.sf', 'milestones')), 'milestones/ missing before sync');

      const result = syncSfStateToWorktree(mainBase, wtBase);

      assert.ok(existsSync(join(wtBase, '.sf', 'milestones')), 'milestones/ created in worktree');
      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001')), 'M001 synced to worktree');
      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001', 'M001-CONTEXT.md')), 'M001 CONTEXT synced');
      assert.ok(existsSync(join(wtBase, '.sf', 'milestones', 'M001', 'M001-ROADMAP.md')), 'M001 ROADMAP synced');
  ...
  console.log('\n=== 8. syncWorktreeStateBack copies tasks/ subdirectory (#1678) ===');
  {
    ...
    try {
      ...
      const wtSliceDir = join(wtBase, '.sf', 'milestones', 'M002', 'slices', 'S01');
      ...
      mkdirSync(join(mainBase, '.sf', 'milestones', 'M002'), { recursive: true });
      ...
      const mainSliceDir = join(mainBase, '.sf', 'milestones', 'M002', 'slices', 'S01');
  ...
  console.log('\n=== 9. syncWorktreeStateBack syncs root-level files (REQUIREMENTS, PROJECT) ===');
  {
    ...
    try {
      mkdirSync(join(mainBase, '.sf', 'milestones', 'M001'), { recursive: true });
      mkdirSync(join(wtBase, '.sf', 'milestones', 'M001'), { recursive: true });

      // Main has original REQUIREMENTS and PROJECT
      writeFileSync(join(mainBase, '.sf', 'REQUIREMENTS.md'), '# Requirements\n## R001');
      writeFileSync(join(mainBase, '.sf', 'PROJECT.md'), '# Project\n## Milestone: M001');

      // Worktree has updated versions (complete-milestone added M002 refs)
      writeFileSync(join(wtBase, '.sf', 'REQUIREMENTS.md'), '# Requirements\n## R001\n## R002 — New req');
      writeFileSync(join(wtBase, '.sf', 'PROJECT.md'), '# Project\n## Milestone: M001\n## Milestone: M002');
      writeFileSync(join(wtBase, '.sf', 'KNOWLEDGE.md'), '# Knowledge\nLearned something.');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Root-level files should be overwritten with worktree versions
      const reqContent = readFileSync(join(mainBase, '.sf', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        reqContent.includes('R002'),
        'REQUIREMENTS.md updated with worktree content',
      );

      const projContent = readFileSync(join(mainBase, '.sf', 'PROJECT.md'), 'utf-8');
      assert.ok(
      ...
      assert.ok(
        existsSync(join(mainBase, '.sf', 'KNOWLEDGE.md')),
  ...
  console.log('\n=== 10. syncWorktreeStateBack syncs all milestone dirs, not just current ===');
  {
    ...
    try {
      ...
      mkdirSync(join(wtBase, '.sf', 'milestones'), { recursive: true });
      ...
      const wtM002Dir = join(wtBase, '.sf', 'milestones', 'M002-abc123');
      ...
      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.sf', 'milestones', 'M001')),
        'M001 missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.sf', 'milestones', 'M002-abc123')),
      ...
      assert.ok(
        !existsSync(join(mainBase, '.sf', 'milestones', 'M001', 'M001-SUMMARY.md')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.sf', 'milestones', 'M002-abc123', 'M002-abc123-CONTEXT.md')),
        'M002 CONTEXT synced to main (next-milestone fix)',
      );
      assert.ok(
        existsSync(join(mainBase, '.sf', 'milestones', 'M002-abc123', 'M002-abc123-ROADMAP.md')),
  ...
  console.log('\n=== 11. complete-milestone creates next-milestone artifacts that survive sync ===');
  {
    ...
    try {
      ...
      const mainM006 = join(mainBase, '.sf', 'milestones', 'M006-589wvh');
      ...
      const wtM006 = join(wtBase, '.sf', 'milestones', 'M006-589wvh');
      ...
      const wtM007 = join(wtBase, '.sf', 'milestones', 'M007-wortc8');
      ...
      assert.ok(
        !existsSync(join(mainBase, '.sf', 'milestones', 'M006-589wvh', 'M006-589wvh-SUMMARY.md')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.sf', 'milestones', 'M007-wortc8', 'M007-wortc8-CONTEXT.md')),
        'M007 CONTEXT synced to main (next-milestone)',
      );
      assert.ok(
        existsSync(join(mainBase, '.sf', 'milestones', 'M007-wortc8', 'M007-wortc8-ROADMAP.md')),
  ...
  console.log('\n=== 12. root files not in worktree are not created in main ===');
  {
    ...
    try {
      ...
      writeFileSync(join(mainBase, '.sf', 'REQUIREMENTS.md'), '# Original');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Main's REQUIREMENTS should be untouched (worktree had nothing to sync)
      const content = readFileSync(join(mainBase, '.sf', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        content === '# Original',
  ...
  console.log('\n=== 13. QUEUE.md and completed-units.json synced from worktree (#1787) ===');
  {
    ...
    try {
      ...
      writeFileSync(join(wtBase, '.sf', 'QUEUE.md'), '# Queue\n- M002 next');
      writeFileSync(
      ...
      assert.ok(
        !existsSync(join(mainBase, '.sf', 'completed-units.json')),
      ...
      const queueContent = readFileSync(join(mainBase, '.sf', 'QUEUE.md'), 'utf-8');
      assert.ok(
      ...
      const cuContent = readFileSync(join(mainBase, '.sf', 'completed-units.json'), 'utf-8');
      assert.ok(
  ...
  console.log('\n=== 14. syncSfStateToWorktree syncs non-standard milestone dir names (#1547) ===');
  {
    ...
    try {
      ...
      const customDir = join(mainBase, '.sf', 'milestones', 'sprint-alpha');
      mkdirSync(customDir, { recursive: true });
      writeFileSync(join(customDir, 'CONTEXT.md'), '# Sprint Alpha Context');

      const suffixDir = join(mainBase, '.sf', 'milestones', 'M001-abc123');
      mkdirSync(suffixDir, { recursive: true });
      writeFileSync(join(suffixDir, 'M001-abc123-CONTEXT.md'), '# M001 Context');

      assert.ok(!existsSync(join(wtBase, '.sf', 'milestones', 'sprint-alpha')), 'sprint-alpha missing before sync');
      assert.ok(!existsSync(join(wtBase, '.sf', 'milestones', 'M001-abc123')), 'M001-abc123 missing before sync');

      const result = syncSfStateToWorktree(mainBase, wtBase);

      assert.ok(
        existsSync(join(wtBase, '.sf', 'milestones', 'sprint-alpha', 'CONTEXT.md')),
        '#1547: non-standard milestone dir "sprint-alpha" synced to worktree',
      );
      assert.ok(
        existsSync(join(wtBase, '.sf', 'milestones', 'M001-abc123', 'M001-abc123-CONTEXT.md')),
  ...
  console.log('\n=== 15. syncWorktreeStateBack syncs non-standard milestone dir names (#1547) ===');
  {
    ...
    try {
      ...
      const wtCustomDir = join(wtBase, '.sf', 'milestones', 'sprint-beta');
      mkdirSync(wtCustomDir, { recursive: true });
      writeFileSync(join(wtCustomDir, 'SUMMARY.md'), '# Sprint Beta Summary');

      assert.ok(
        !existsSync(join(mainBase, '.sf', 'milestones', 'sprint-beta')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.sf', 'milestones', 'sprint-beta', 'SUMMARY.md')),
