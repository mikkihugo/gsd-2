...
describe("#2883: isToolInvocationError classification", () => {
  test("detects JSON validation failure pattern", () => {
    assert.equal(
      isToolInvocationError("Validation failed for tool sf_complete_slice: Expected ',' or '}' in JSON"),
      true,
    );
  });
  ...
  test("detects 'Validation failed for tool' prefix", () => {
    assert.equal(
      isToolInvocationError("Validation failed for tool sf_slice_complete"),
      true,
    );
  });
