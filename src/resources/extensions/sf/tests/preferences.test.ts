...
test("experimental.rtk: non-boolean produces error", () => {
  const result = validatePreferences({ experimental: { rtk: "yes" } } as unknown as SFPreferences);
...
test("experimental: non-object produces error", () => {
  const result = validatePreferences({ experimental: true } as unknown as SFPreferences);
  assert.ok(result.errors.some(e => e.includes("experimental must be an object")));
});

test("experimental: unknown key produces warning", () => {
  const result = validatePreferences({ experimental: { rtk: true, future_flag: true } } as unknown as SFPreferences);
