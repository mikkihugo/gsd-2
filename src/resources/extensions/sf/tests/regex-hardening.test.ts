...
async function main(): Promise<void> {
  ...
  //     Pattern: /^sf\/(?:([a-zA-Z0-9_-]+)\/)?(M\d+(?:-[a-z0-9]{6})?)\/(S\d+)$/
  {
    ...
    {
      const m = 'sf/M001/S01'.match(SLICE_BRANCH_RE);
      assertTrue(m !== null, 'matches sf/M001/S01');
      assertEq(m?.[1], undefined, 'no worktree prefix for sf/M001/S01');
    ...
    {
      const m = 'sf/M001-abc123/S01'.match(SLICE_BRANCH_RE);
      assertTrue(m !== null, 'matches sf/M001-abc123/S01');
