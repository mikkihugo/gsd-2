...
import { openDatabase, closeDatabase, insertMilestone, insertSlice } from "../sf-db.ts";
...
function setupRepo(): string {
  ...
  openDatabase(join(repo, ".sf", "sf.db"));
  return repo;
}
...
test("dispatch guard blocks when prior milestone has incomplete slices", (t) => {
  ...
  mkdirSync(join(repo, ".sf", "milestones", "M002"), { recursive: true });
  mkdirSync(join(repo, ".sf", "milestones", "M003"), { recursive: true });
...
test("dispatch guard ignores positionally-earlier reverse dependents for zero-dependency slices (#3720)", (t) => {
  ...
  mkdirSync(join(repo, ".sf", "milestones", "M015"), { recursive: true });
...
test("dispatch guard treats zero-dependency slices as independent when a milestone uses explicit deps (#3998)", (t) => {
  ...
  mkdirSync(join(repo, ".sf", "milestones", "M022"), { recursive: true });
...
test("dispatch guard skips cross-milestone check when SF_MILESTONE_LOCK is set (#2797)", (t) => {
  ...
  mkdirSync(join(repo, ".sf", "milestones", "M010"), { recursive: true });
  mkdirSync(join(repo, ".sf", "milestones", "M011"), { recursive: true });
  mkdirSync(join(repo, ".sf", "milestones", "M012"), { recursive: true });
