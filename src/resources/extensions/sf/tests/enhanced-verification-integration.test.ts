 ...
 *
 * Exercises all 7 enhanced verification checks against SF-2's actual source files.
 * This proves:
...
function verifyTestFixturesExist(): void {
  for (const file of REAL_SF_FILES) {
...
4. Run tests to verify changes
          `.trim(),
          files: REAL_SF_FILES.slice(0, 4).map((f) => join(SF_SRC_DIR, f)),
          inputs: [
            join(SF_SRC_DIR, "types.ts"),
            join(SF_SRC_DIR, "errors.ts"),
          ],
          expected_output: [
            join(SF_SRC_DIR, "sf-db.ts"),
          ],
        }),
      ];
    ...
    test("handles multi-task sequence with file dependencies", async () => {
      ...
      const tasks: TaskRow[] = [
        ...
        createTask({
          ...
          description: `
Read the types from src/resources/extensions/sf/types.ts and use them.
          `.trim(),
          inputs: [join(SF_SRC_DIR, "types.ts")],
          files: [join(SF_SRC_DIR, "sf-db.ts")],
        }),
      ];
    ...
    test("handles task sequence with signature analysis", () => {
      ...
      const priorTasks: TaskRow[] = [
        createTask({
          ...
          status: "complete",
          key_files: [join(SF_SRC_DIR, "sf-db.ts")],
        }),
      ];
