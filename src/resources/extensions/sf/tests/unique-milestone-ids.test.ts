...
describe('unique-milestone-ids', async () => {
  ...
  // (h) Preferences round-trip via renderPreferencesForSystemPrompt
  {
    ...
    const prefsTrue: SFPreferences = { unique_milestone_ids: true };
    ...
    const prefsUndefined: SFPreferences = {};
    ...
    const prefsFalse: SFPreferences = { unique_milestone_ids: false };
    ...
    const prefsCoerced: SFPreferences = { unique_milestone_ids: 1 as unknown as boolean };
    ...
    // SFPreferences interface accepts the field (compile-time check — if this compiles, it works)
    const prefs: SFPreferences = { unique_milestone_ids: true, version: 1 };
    assert.ok(prefs.unique_milestone_ids === true, 'SFPreferences interface accepts unique_milestone_ids');
  }
});
