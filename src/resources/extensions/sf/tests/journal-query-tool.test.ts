...
test("registerJournalTools registers sf_journal_query tool", () => {
  const pi = makeMockPi();
  registerJournalTools(pi);
  assert.equal(pi.tools.length, 1, "Should register exactly one tool");
  assert.equal(pi.tools[0].name, "sf_journal_query");
});

// ─── Filtering ────────────────────────────────────────────────────────────────

test("sf_journal_query returns filtered entries", async () => {
...
test("sf_journal_query returns 'no entries' message for empty results", async () => {
  const base = makeTmpBase();
  try {
...
test("sf_journal_query respects limit parameter", async () => {
  const base = makeTmpBase();
  try {
    for (let i = 0; i < 5; i++) {
...
test("sf_journal_query handles errors gracefully", async () => {
  const pi = makeMockPi();
  registerJournalTools(pi);
  const tool = pi.tools[0];
