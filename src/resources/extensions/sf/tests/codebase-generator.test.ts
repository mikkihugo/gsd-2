...
test("generateCodebaseMap: excludes .sf/ files", () => {
  const base = makeTmpRepo();
  try {
    addFile(base, "src/main.ts");
    addFile(base, ".sf/PROJECT.md");
...
test("writeCodebaseMap: creates .sf/ directory if missing", () => {
  const base = join(tmpdir(), `sf-codebase-test-${randomUUID()}`);
  mkdirSync(base, { recursive: true });
  // Intentionally do NOT pre-create .sf/
  try {
