...
describe("context-store: queryArtifact", () => {
  ...
  test("returns content for existing path", () => {
    ...
    insertArtifact({
      path: '.sf/milestones/M001/M001-PLAN.md',
    ...
    const plan = queryArtifact('.sf/milestones/M001/M001-PLAN.md');
...
`;
  ...
  test("S02 with S01 predecessor includes both rows", () => {
    const result = formatRoadmapExcerpt(sampleRoadmap, 'S02', '.sf/milestones/M005/M005-ROADMAP.md');
