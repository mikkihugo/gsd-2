...
function writeAgent(root: string, configDirName: ".sf" | ".pi", name = "ping"): string {
...
test("discoverAgents finds project agents in .sf/agents", (t) => {
	const root = makeProjectRoot(t);
	const agentsDir = writeAgent(root, ".sf");
...
test("discoverAgents accepts tools frontmatter as a YAML list", (t) => {
	const root = makeProjectRoot(t);
	const agentsDir = join(root, ".sf", "agents");
	mkdirSync(agentsDir, { recursive: true });
	writeFileSync(
