...
describe('git-service', async () => {
  ...
  test('Branch queries', () => {
    ...
    run("git checkout -b sf/M001/S01", repo);
    assert.deepStrictEqual(svc.getCurrentBranch(), "sf/M001/S01", "getCurrentBranch returns slice branch name");
  ...
  test('createSnapshot: enabled by default when prefs omitted', () => {
    ...
    svc.createSnapshot("sf/M001/S01");

    // Verify ref exists under refs/sf/snapshots/
    const refs = run("git for-each-ref refs/sf/snapshots/", repo);
    assert.ok(refs.includes("refs/sf/snapshots/sf/M001/S01/"), "snapshot ref created under refs/sf/snapshots/");
  ...
  test('Integration branch: rejects slice branches', () => {
    ...
    writeIntegrationBranch(repo, "M001", "sf/M001/S01");
  ...
  test('MergeConflictError: constructor fields', () => {
    ...
    assert.deepStrictEqual(err.branch, "sf/M001/S01", "MergeConflictError.branch set");
  ...
  test('Integration branch: rejects gsd/quick/* branches', () => {
    ...
    assert.deepStrictEqual(readIntegrationBranch(repo, "M001"), null, "sf/quick/* branches are not recorded as integration branch");
  ...
  // ─── autoCommit: absorbs preceding sf snapshot commits ─────────────────

  test('autoCommit: absorbs preceding sf snapshot commits', () => {
    const repo = initTempRepo();

    // Simulate 2 sf snapshot commits
    createFile(repo, "file1.ts", "v1");
    run("git add -A", repo);
    run('git commit -m "sf snapshot: uncommitted changes after 35m inactivity"', repo);

    createFile(repo, "file2.ts", "v2");
    run("git add -A", repo);
    run('git commit -m "sf snapshot: pre-dispatch, uncommitted changes after 40m inactivity"', repo);
    ...
    // No sf snapshot commits in log
    const log = run("git log --oneline", repo);
    assert.ok(!log.includes("sf snapshot"), "no sf snapshot commits remain in history");
