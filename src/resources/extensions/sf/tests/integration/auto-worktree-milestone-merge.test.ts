...
describe("auto-worktree-milestone-merge", { timeout: 300_000 }, () => {
  ...
  test("basic squash merge — one commit on main", () => {
    ...
    const worktreeDir = join(repo, ".sf", "worktrees", "M010");
  ...
  test("auto-resolve .sf/ state file conflicts", () => {
    const repo = freshRepo();
    ...
    try {
      ...
      assert.ok(result.commitMessage.includes("feat:") && result.commitMessage.includes("SF-Milestone: M050"), "merge commit created despite .sf conflict");
    } catch (err) {
      threw = true;
    }
    assert.ok(!threw, "auto-resolves .sf/ state file conflicts without throwing");
  ...
  test("master-branch repo — no META.json, no prefs (#1668)", () => {
    ...
    const metaFile = join(dir, ".sf", "milestones", "M070", "M070-META.json");
  ...
  test("#1738 bug 3: synced .sf/ dirs cleaned before merge", () => {
    ...
    const msDir = join(repo, ".sf", "milestones", "M090", "slices", "S01");
    mkdirSync(msDir, { recursive: true });
    writeFileSync(join(msDir, "S01-PLAN.md"), "# synced plan\n");
    writeFileSync(
      join(repo, ".sf", "milestones", "M090", "M090-ROADMAP.md"),
      "# synced roadmap\n",
    );

    const runtimeDir = join(repo, ".sf", "runtime", "units");
    ...
    assert.ok(!threw, "#1738 merge does not fail on synced .sf/ files");
  ...
  test("#1906: codeFilesChanged=false when only .sf/ metadata merged", () => {
    const repo = freshRepo();
    const wtPath = createAutoWorktree(repo, "M180");

    mkdirSync(join(wtPath, ".sf", "milestones", "M180"), { recursive: true });
    writeFileSync(
      join(wtPath, ".sf", "milestones", "M180", "SUMMARY.md"),
    ...
    assert.strictEqual(result.codeFilesChanged, false,
      "#1906: codeFilesChanged must be false when only .sf/ files were merged");
  });
  ...
  test("#2156: mergeMilestoneToMain removes external-state worktrees using the milestone branch name", () => {
    ...
    assert.ok(
      ...
      `worktree should be registered under external .sf state, got ${realWtPath}`,
    );

    // Recreate the exact divergence from #1852: local .sf/ is replaced with a
    // stale real directory, so worktreePath() no longer matches git's record.
    unlinkSync(join(repo, ".sf"));
    mkdirSync(join(repo, ".sf", "worktrees", "M215"), { recursive: true });
    writeFileSync(join(repo, ".sf", "STATE.md"), "# Local stale state\n");
