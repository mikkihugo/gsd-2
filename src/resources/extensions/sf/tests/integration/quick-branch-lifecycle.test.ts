...
test('QUICK_BRANCH_RE: matches quick-task branches', () => {
  assert.ok(QUICK_BRANCH_RE.test("sf/quick/1-fix-typo"), "matches standard quick branch");
});

  assert.ok(QUICK_BRANCH_RE.test("sf/quick/42-some-long-slug-name"), "matches multi-digit quick branch");
  assert.ok(!QUICK_BRANCH_RE.test("main"), "rejects main");
  assert.ok(!QUICK_BRANCH_RE.test("sf/M001/S01"), "rejects slice branch");
  assert.ok(!QUICK_BRANCH_RE.test("sf/quickly-something"), "rejects non-quick prefix");
  assert.ok(!QUICK_BRANCH_RE.test("feature/sf/quick/1"), "rejects nested prefix");
  // ═══════════════════════════════════════════════════════════════════════
  // captureIntegrationBranch: guard against quick-task branches
  // ═══════════════════════════════════════════════════════════════════════
...
test('cleanupQuickBranch: merges back and cleans up (same session)', async () => {
    ...
    const returnState = {
      basePath: repo,
      originalBranch: "main",
      quickBranch: "sf/quick/1-fix-typo",
      taskNum: 1,
      slug: "fix-typo",
    ...
    assert.ok(!branches.includes("sf/quick/1-fix-typo"), "quick branch deleted");
...
test('cleanupQuickBranch: recovers from disk state (cross-session)', async () => {
    ...
    writeFileSync(join(runtimeDir, "quick-return.json"), JSON.stringify({
      basePath: repo,
      originalBranch: "main",
      quickBranch: "sf/quick/2-add-docs",
      taskNum: 2,
      slug: "add-docs",
