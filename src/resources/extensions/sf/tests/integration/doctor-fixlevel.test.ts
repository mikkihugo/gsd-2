...
/**
 * Build a minimal .sf structure: milestone with one slice, one task
...
test("doctor source treats skipped DB slices as closed and directory-optional", () => {
  const doctorSource = readFileSync(join(process.cwd(), "src/resources/extensions/sf/doctor.ts"), "utf8");
  assert.match(
    doctorSource,
