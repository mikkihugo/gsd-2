...
describe('doctor-proactive', async () => {
  ...
  try {
    ...
    test('health gate: missing STATE.md does NOT block dispatch (#889)', async () => {
      ...
      writeFileSync(join(dir, ".sf", "milestones", "M001", "M001-ROADMAP.md"), "# Roadmap\n");
    ...
    test('health gate: stale crash lock auto-cleared', async () => {
      ...
      // Write a stale lock
      writeFileSync(join(dir, ".sf", "auto.lock"), JSON.stringify({
      ...
      assert.ok(!existsSync(join(dir, ".sf", "auto.lock")), "lock file removed");
    });
    ...
    test('health gate: STATE.md missing — auto-healed', async () => {
      const dir = realpathSync(mkdtempSync(join(tmpdir(), "doc-proactive-")));
      cleanups.push(dir);
      // Minimal .sf structure: milestones dir exists but no STATE.md
      mkdirSync(join(dir, ".sf", "milestones"), { recursive: true });

      const stateFile = join(dir, ".sf", "STATE.md");
