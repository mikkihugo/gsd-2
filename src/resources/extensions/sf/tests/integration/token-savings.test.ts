...
import { openDatabase, closeDatabase } from '../../sf-db.ts';
...
console.log('\n=== token-savings: plan-slice prompt ≥30% character savings ===');
{
  ...
  writeFileSync(join(base, '.sf', 'DECISIONS.md'), decisionsMarkdown);
  writeFileSync(join(base, '.sf', 'REQUIREMENTS.md'), requirementsMarkdown);
  writeFileSync(join(base, '.sf', 'PROJECT.md'), PROJECT_CONTENT);
  ...
  const fullDecisionsContent = readFileSync(join(base, '.sf', 'DECISIONS.md'), 'utf-8');
  const fullRequirementsContent = readFileSync(join(base, '.sf', 'REQUIREMENTS.md'), 'utf-8');
