...
test('(k) run-uat prompt template', () => {
  ...
  const sliceId = 'S01';
  const uatPath = '.sf/milestones/M001/slices/S01/S01-UAT.md';
  const uatResultPath = '.sf/milestones/M001/slices/S01/S01-ASSESSMENT.md';
...
test('(k2) run-uat prompt references sf_summary_save, not direct write', () => {
  const promptResult = loadPromptFromWorktree('run-uat', {
    workingDirectory: '/tmp/test-project',
    milestoneId: 'M001',
    sliceId: 'S01',
    uatPath: '.sf/milestones/M001/slices/S01/S01-UAT.md',
    uatResultPath: '.sf/milestones/M001/slices/S01/S01-UAT.md',
    uatType: 'artifact-driven',
    inlinedContext: '<!-- no context -->',
  });

  assert.ok(
    promptResult.includes('sf_summary_save'),
    'run-uat prompt should reference sf_summary_save tool',
  );
  assert.ok(
...
test('(m) non-artifact UAT skip', async () => {
    const base = createFixtureBase();
    try {
      const roadmapDir = join(base, '.sf', 'milestones', 'M001');
...
test('(q) verdict in ASSESSMENT file skips UAT dispatch (file-based path)', async () => {
    ...
    // S{sid}-ASSESSMENT.md (via sf_summary_save artifact_type:"ASSESSMENT"),
