...
test('── markdown-renderer: renderRoadmapCheckboxes round-trip ──', async () => {
  ...
  try {
    ...
    const roadmapPath = path.join(tmpDir, '.sf', 'milestones', 'M001', 'M001-ROADMAP.md');
...
test('── markdown-renderer: renderPlanCheckboxes round-trip ──', async () => {
  ...
  try {
    ...
    const planPath = path.join(tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md');
  ...
  try {
    ...
    insertTask({
      ...
      planning: {
        ...
        expectedOutput: ['src/resources/extensions/sf/tests/auto-recovery.test.ts'],
    ...
    assert.strictEqual(parsedPlan.tasks[0].files?.[0], 'src/resources/extensions/sf/markdown-renderer.ts', 'files list preserved in slice plan');
    ...
    const taskPlanPath = path.join(tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S02', 'tasks', 'T01-PLAN.md');
...
test('── markdown-renderer: renderTaskSummary round-trip ──', async () => {
  ...
  try {
    ...
    const summaryPath = path.join(
      tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S01', 'tasks', 'T01-SUMMARY.md',
    );
...
test('── markdown-renderer: renderSliceSummary round-trip ──', async () => {
  ...
  try {
    ...
    // Verify SUMMARY file
    const summaryPath = path.join(
      tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-SUMMARY.md',
    );
    ...
    const uatPath = path.join(
      tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-UAT.md',
    );
...
test('── markdown-renderer: renderAllFromDb produces all files ──', async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md'),
      plan1,
    );
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S02', 'S02-PLAN.md'),
      plan2,
    );
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.sf', 'milestones', 'M002', 'slices', 'S01', 'S01-PLAN.md'),
      plan3,
    );
    ...
    const m1Roadmap = fs.readFileSync(
      path.join(tmpDir, '.sf', 'milestones', 'M001', 'M001-ROADMAP.md'), 'utf-8',
    );
    clearAllCaches();
    ...
    const m1s1Plan = fs.readFileSync(
      path.join(tmpDir, '.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md'), 'utf-8',
    );
    clearAllCaches();
