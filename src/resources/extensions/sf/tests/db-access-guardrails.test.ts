...
test("system.md anti-patterns section prohibits direct .gsd/gsd.db access", () => {
  ...
  assert.match(prompt, /sf_\*/, "system.md DB guardrail must redirect to sf_* tools");
});
...
test("reassess-roadmap.md contains DB access safety guardrail", () => {
  ...
  assert.match(prompt, /sf_milestone_status/, "reassess-roadmap.md must name sf_milestone_status as alternative");
});
