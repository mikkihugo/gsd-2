...
function makeTmpBase(): string {
  ...
  mkdirSync(join(base, '.sf', 'milestones', 'M001', 'slices', 'S02'), { recursive: true });
  mkdirSync(join(base, '.sf', 'milestones', 'M001', 'slices', 'S03'), { recursive: true });
  return base;
}
...
test('handleReassessRoadmap succeeds when modifying only pending slices', async () => {
  ...
  try {
    ...
    const assessmentPath = join('.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-ASSESSMENT.md');
    ...
    const assessmentDiskPath = join(base, '.sf', 'milestones', 'M001', 'slices', 'S01', 'S01-ASSESSMENT.md');
...
test('handleReassessRoadmap invalidates stale milestone-validation when roadmap changes (#2957)', async () => {
  ...
  try {
    ...
    const validationPath = join('.sf', 'milestones', 'M001', 'M001-VALIDATION.md');
    insertAssessment({
      path: validationPath,
      milestoneId: 'M001',
      sliceId: null,
