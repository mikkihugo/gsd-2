...
export function loadEffectiveSFPreferences(): LoadedSFPreferences | null {
  ...
  let result: LoadedSFPreferences;
  ...
  if (profile) {
    ...
    result = {
      ...result,
      preferences: mergePreferences(profileDefaults as SFPreferences, result.preferences),
    };
  }
...
export function parsePreferencesMarkdown(content: string): SFPreferences | null {
...
function parseFrontmatterBlock(frontmatter: string): SFPreferences {
  try {
    const parsed = parseYaml(frontmatter);
    if (typeof parsed !== 'object' || parsed === null) {
      return {} as SFPreferences;
    }
    return parsed as SFPreferences;
  } catch (e) {
 ...
 *   - planner: sonnet
 */
function parseHeadingListFormat(content: string): SFPreferences {
 ...
 */
export function applyModeDefaults(mode: WorkflowMode, prefs: SFPreferences): SFPreferences {
  const defaults = MODE_DEFAULTS[mode];
  if (!defaults) return prefs;
  return mergePreferences(defaults, prefs);
}

function mergePreferences(base: SFPreferences, override: SFPreferences): SFPreferences {
  return {
...
export function renderPreferencesForSystemPrompt(preferences: SFPreferences, resolutions?: Map<string, SkillResolution>): string {
...
export function resolveParallelConfig(prefs: SFPreferences | undefined): import("./types.js").ParallelConfig {
  return {
