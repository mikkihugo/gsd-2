...
export async function handlePrefs(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (trimmed === "status") {
    ...
    const projectStatus = projectPrefs ? `present: ${projectPrefs.path}` : `missing: ${getProjectSFPreferencesPath()}`;
...
export async function handlePrefsMode(ctx: ExtensionCommandContext, scope: "global" | "project"): Promise<void> {
  const path = scope === "project" ? getProjectSFPreferencesPath() : getGlobalSFPreferencesPath();
  const existing = scope === "project" ? loadProjectSFPreferences() : loadGlobalSFPreferences();
