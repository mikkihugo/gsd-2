...
export function registerDbTools(pi: ExtensionAPI): void {
  ...
  const decisionSaveExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    try {
      ...
      logError("tool", `sf_decision_save tool failed: ${msg}`, { tool: "sf_decision_save", error: String(err) });
      return {
  ...
  const requirementUpdateExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `sf_requirement_update tool failed: ${msg}`, { tool: "sf_requirement_update", error: String(err) });
      return {
  ...
  registerAlias(pi, requirementUpdateTool, "sf_update_requirement", "sf_requirement_update");
  ...
  const requirementSaveExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `sf_requirement_save tool failed: ${msg}`, { tool: "sf_requirement_save", error: String(err) });
      return {
  ...
  registerAlias(pi, requirementSaveTool, "sf_save_requirement", "sf_requirement_save");
  ...
  // ─── sf_milestone_generate_id (formerly sf_generate_milestone_id) ────
  ...
  registerAlias(pi, milestoneGenerateIdTool, "sf_generate_milestone_id", "sf_milestone_generate_id");
  ...
  registerAlias(pi, planMilestoneTool, "sf_milestone_plan", "sf_plan_milestone");
  ...
  const planTaskExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `plan_task tool failed: ${msg}`, { tool: "sf_plan_task", error: String(err) });
      return {
  ...
  registerAlias(pi, sliceCompleteTool, "sf_complete_slice", "sf_slice_complete");
  ...
  const skipSliceExecute = async (_toolCallId: string, params: any, _signal: AbortSignal | undefined, _onUpdate: unknown, _ctx: unknown) => {
    ...
    try {
      ...
      try {
        ...
        logError("tool", `skip_slice rebuildState failed: ${(err as Error).message}`, { tool: "sf_skip_slice" });
      }
      ...
      const msg = err instanceof Error ? err.message : String(err);
      logError("tool", `skip_slice tool failed: ${msg}`, { tool: "sf_skip_slice", error: String(err) });
      return {
  ...
  registerAlias(pi, milestoneCompleteTool, "sf_milestone_complete", "sf_complete_milestone");
  ...
  registerAlias(pi, milestoneValidateTool, "sf_milestone_validate", "sf_validate_milestone");
  ...
  registerAlias(pi, reassessRoadmapTool, "sf_roadmap_reassess", "sf_reassess_roadmap");
