 ...
 */
export function resolveProfileDefaults(profile: TokenProfile): Partial<SFPreferences> {
  switch (profile) {
    case "budget":
      return {
        models: {
 ...
 */
export function resolveSearchProviderFromPreferences(): SFPreferences["search_provider"] | undefined {
  const prefs = loadEffectiveSFPreferences();
