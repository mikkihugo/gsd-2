...
export async function executeSummarySave(
  ...
  try {
    ...
    logError("tool", `sf_summary_save tool failed: ${msg}`, { tool: "sf_summary_save", error: String(err) });
    return {
...
export async function executeTaskComplete(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `complete_task tool failed: ${msg}`, { tool: "sf_task_complete", error: String(err) });
    return {
...
export async function executeSliceComplete(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `complete_slice tool failed: ${msg}`, { tool: "sf_slice_complete", error: String(err) });
    return {
...
export async function executeCompleteMilestone(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `complete_milestone tool failed: ${msg}`, { tool: "sf_complete_milestone", error: String(err) });
    return {
...
export async function executeValidateMilestone(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `validate_milestone tool failed: ${msg}`, { tool: "sf_validate_milestone", error: String(err) });
    return {
...
export async function executeReassessRoadmap(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `reassess_roadmap tool failed: ${msg}`, { tool: "sf_reassess_roadmap", error: String(err) });
    return {
...
export async function executeSaveGateResult(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `sf_save_gate_result failed: ${msg}`, { tool: "sf_save_gate_result", error: String(err) });
    return {
...
export async function executePlanMilestone(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `plan_milestone tool failed: ${msg}`, { tool: "sf_plan_milestone", error: String(err) });
    return {
...
export async function executePlanSlice(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `plan_slice tool failed: ${msg}`, { tool: "sf_plan_slice", error: String(err) });
    return {
...
export async function executeReplanSlice(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logError("tool", `replan_slice tool failed: ${msg}`, { tool: "sf_replan_slice", error: String(err) });
    return {
