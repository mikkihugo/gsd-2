...
const TEST_AUTO_DASHBOARD_FALLBACK_ENV = "SF_WEB_TEST_USE_FALLBACK_AUTO_DASHBOARD";
const AUTO_DASHBOARD_MODULE_ENV = "SF_AUTO_DASHBOARD_MODULE";
 ...
 *   1. .sf/auto.lock — if present and its PID is alive, auto IS running.
 *   2. .sf/runtime/paused-session.json — if present, auto IS paused.
 *
 * See #2705.
 */
function reconcileWithDiskState(
  ...
  const lockPath = join(projectCwd, ".sf", "auto.lock");
    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        ...
        try {
          ...
          const projectCwd = env.SF_WEB_PROJECT_CWD || "";
